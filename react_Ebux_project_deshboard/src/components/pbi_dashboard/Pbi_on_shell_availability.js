...
import { shelf_availability } from "../../services/pbi_dashboard/pbi_dashboard_api";
import { summary } from "../../services/pbi_dashboard/pbi_dashboard_api";
import { shelf_availability_filter } from "../../services/pbi_dashboard/pbi_dashboard_api";
...
function Pbi_on_shell_availability() {
  ...
  const handleRegistration = async (data) => {
    ...
    data["keys"] = "";
    let resp = await shelf_availability_filter(token, data);
  ...
  //first time api hit
  const getData = async () => {
    const res = await shelf_availability(token);
    ...
    const uniquedate = Array.from(
      new Set(res.data.avail_data.map((values) => values.created_on))
    );
    // console.log(uniquedate);
    setCreated(uniquedate);
    // for (let l = 0; l < res.data.avail_data.length; l++) {
    //   if (
    //     res.data.avail_data[l].platform_name ===
    //     ("Bigbasket" ||
    //       "Blinkit" ||
    //       "Flipkart Supermart" ||
    //       "Amazon" ||
    ...
    //     setCreated((created) => [
    //       ...created,
    //       res.data.avail_data[l].created_on,
    //     ]);
    //   }
    // }
  ...
  const loopFunction = (resp) => {
    ...
    setLocation([]);
    setLocationdata([]);
    resp.data.loc_avail.map((locations) => {
      setLocation((location) => [...location, locations.location_name]);
      setLocationdata((locationdata) => [
        ...locationdata,
        locations.availability * 100,
      ]);
    });
    ...
    resp.data.brand_avail.map((key_word) => {
      setKeywords((keywords) => [...keywords, key_word.brand_name]);
      setKeywordsdata((keywordsdata) => [
        ...keywordsdata,
        key_word.availability * 100,
      ]);
    });

    setPlatform([]);
    const uniqueplateform = Array.from(
      new Set(resp.data.avail_data.map((values) => values.platform_name))
    );
    setPlatform(uniqueplateform);

    // resp.data.avail_data
    //   .filter((value) => {
    //     return value.created_on === resp.data.avail_data[0].created_on;
    //   })
    //   .map((value) =>
    ...
    uniqueplateform.map((values) => {
      const data_value = resp.data.avail_data
        .filter((data) => data.platform_name === values)
        .map((value) => (value.availability * 100).toFixed(0));
      console.log(data_value);
      setCreateddata1((createdData1) => [...createdData1, data_value]);
    });
    // for (var i = 0; i < resp.data.avail_data.length / 3; i++) {
    //   resp.data.avail_data
    //     .filter((value) => {
    //       return value.platform_name === resp.data.avail_data[i].platform_name;
    //     })
    //     .map((value) =>
  ...
  const bar_hor_data = {
    ...
    datasets: [
      {
        label: error ? "No Data Found " : "Share of search top 10 keyword ",
        ...
        <div className="container mt-2">
          <div className="row" style={{ textAlign: "center" }}>
            ...
            >
              ...
              <Link to={"#"}>On-Shelf-Availability</Link>
            </div>
          </div>
          {/* <div className="row">
            <div
                      ...
                      {cardresponse &&
                        cardresponse.category_data.map((value, index) => (
                          <div style={{ width: 100 }} key={index}>
                            <CircularProgressbar
                              value={(value.availability * 100).toFixed(0)}
                              text={`${(value.availability * 100).toFixed(0)}%`}
                              styles={buildStyles({
              ...
              >
                ...
                >
                  <h4 className="card-title text-center p-2 mb-1 bg-primary text-white">
                    On Shelf Availability Trend by Platform
                  </h4>
              ...
              <div className="row">
                <div className="col-lg-6">
                  <div className="card p-0">
                    <h4 className="card-title text-center p-2 mb-1 bg-primary text-white">
                      On Shell Availability By Brand
                    </h4>
                    <div className="card-body">
                ...
                <div className="col-lg-6">
                  <div className="card">
                    <h4 className="card-title text-center p-2 mb-1 bg-primary text-white">
                      location wise on Shell Availability
                    </h4>
                    <div className="card-body">
                      <Bar data={linedata} />
                    </div>
...
export default Pbi_on_shell_availability;
